...
function yt_dash_setup() {
	if ( current_user_can( get_option( 'yt_dash_access' ) ) ) {
		wp_add_dashboard_widget( 'yt_dash-widget', 'YouTube Analytics Dashboard', 'yt_dash_content', $control_callback = null );
	}
}
...
function yt_dash_content() {
	...
	switch ( $yt_query ) {
		...
		case 'comments' :
			$title = __( "Coments", 'yt_dash' );
			break;
		
		case 'averageViewDuration' :
			$title = __( "Average View Duration (seconds)", 'yt_dash' );
			break;
		
		case 'likes' :
			$title = __( "Likes", 'yt_dash' );
			break;
		
		case 'dislikes' :
			$title = __( "Dislikes", 'yt_dash' );
			break;
		
		case 'estimatedMinutesWatched' :
			$title = __( "Minutes Watched (minutes)", 'yt_dash' );
			break;
		
		default :
			$title = __( "Views", 'yt_dash' );
	}
    ...
    <script type="text/javascript">
      ...
      function yt_dash_drawstats() {
        var data = google.visualization.arrayToDataTable([' . "
          ['" . __( "Date", 'yt_dash' ) . "', '" . $title . "']," . $yt_dash_statsdata . "  
        ]);
	...
	if ( get_option( 'yt_dash_additional' ) ) {
		...
		$code .= '
			...
			var data = google.visualization.arrayToDataTable([' . "
			  ['" . __( "Statistics", 'yt_dash' ) . "', '" . __( "Value", 'yt_dash' ) . "']," . $yt_dash_additional . "  
			]);
